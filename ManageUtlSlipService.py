...
def main():
    ...
    while(pageNo < maxPage):
        pageNo += 1
        url = "http://apis.data.go.kr/1192000/openapi/service/ManageUtlSlipService/getUtlSlipList?ServiceKey={}&pageNo={}&numOfRows=10&type=json&baseDt=20160301".format(serviceKey, pageNo)
        ...
        if(rescode==200):
            try:
                ...
                datalist = response_body['responseJson']['body']['item']
                df = json_normalize(datalist)
                df.to_sql("ManageUtlSlipService", engine, if_exists='append', index=False, chunksize=1000)
